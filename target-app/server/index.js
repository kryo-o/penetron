...
app.get("/api/search", (req, res) => {
    ...
    const sql = `SELECT id, name, category, price FROM products WHERE name LIKE '%${q}%' ORDER BY price ${sort}`;
    try {
...
app.get("/api/my/orders", authMiddleware, (req, res) => {
    res.json(
        db.prepare("SELECT * FROM orders WHERE user_id = ?").all(req.user.sub)
    );
});
...
app.use((req, res, next) => {
    if (
        ...
        return res.sendFile(path.join(clientDist, "index.html"), (err) => {
            if (err) {
                res.status(200).send(
                    "Penetron target app API is running. Build the client with `npm run build:client`."
                );
            }
        });
    }
...
if (require.main === module) {
    app.listen(PORT, () => {
        console.log(
            `Penetron target app listening on http://localhost:${PORT}`
        );
    });
}
